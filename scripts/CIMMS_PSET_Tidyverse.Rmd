...
## A2. Missingness overview
1. For each column, compute the number of missing values.
2. Which 3 columns have the most missing values?

*Hint:* `summarise(across(...))` + `sort()` / `pivot_longer()`.
...
3. Create a two-way count table of `Condition` by `Medication`.
...
## C5. Data quality checks (hard)
Write code to answer each:

1. Are there any patients with **multiple different genders** recorded across rows?
2. Are there any patients with **multiple different emails** recorded across rows?
3. Which patients have the **highest number of visits**? Return the top 5.
